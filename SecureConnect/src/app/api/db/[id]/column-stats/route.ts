...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  const a = await authorize(req, params.id, "db.stats", {
    rateLimitMax: 60,
  ...
  if (!database || !table || !column) {
    return jerr("BAD_REQUEST", "Missing database/table/column", 400);
  }
  ...
  try {
    ...
    return jerr(
      "STATS_FAIL",
      isValidation ? msg : "Failed to compute stats",
