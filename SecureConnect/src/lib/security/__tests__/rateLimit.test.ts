...
describe("rateLimit", () => {
  ...
  test("allows up to limit", async () => {
    for (let i = 0; i < 5; i++) {
      const r = await rateLimit("a", 5, 60_000);
      expect(r.ok).toBe(true);
    }
  });

  test("blocks after limit reached", async () => {
    for (let i = 0; i < 5; i++) await rateLimit("b", 5, 60_000);
    const r = await rateLimit("b", 5, 60_000);
  ...
  test("isolates keys", async () => {
    for (let i = 0; i < 3; i++) await rateLimit("c1", 3, 60_000);
    expect((await rateLimit("c1", 3, 60_000)).ok).toBe(false);
    expect((await rateLimit("c2", 3, 60_000)).ok).toBe(true);
  });

  test("recovers after window passes", async () => {
    for (let i = 0; i < 3; i++) await rateLimit("d", 3, 50);
    expect((await rateLimit("d", 3, 50)).ok).toBe(false);
    await new Promise((res) => setTimeout(res, 80));
    expect((await rateLimit("d", 3, 50)).ok).toBe(true);
  });

  test("reset clears specific key", async () => {
    for (let i = 0; i < 5; i++) await rateLimit("e", 5, 60_000);
    expect((await rateLimit("e", 5, 60_000)).ok).toBe(false);
    resetRateLimit("e");
    expect((await rateLimit("e", 5, 60_000)).ok).toBe(true);
  });
});
